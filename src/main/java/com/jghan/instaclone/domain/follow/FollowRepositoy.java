...
public interface FollowRepositoy extends JpaRepository<Follow,Integer>{
    ...
    void mFollow(@Param("fromUserId") int fromUserId, @Param("toUserId") int toUserId);

    @Modifying
    @Query(value = "DELETE FROM follow WHERE fromUserId = :fromUserId AND toUserId = :toUserId", nativeQuery = true)
    void mUnFollow(@Param("fromUserId") int fromUserId, @Param("toUserId") int toUserId);
    ...
    int mFollowState(@Param("principalId") int principalId, @Param("pageUserId") int pageUserId);

    //해당 페이지의 유저가 팔로우 하는 수
    @Query(value = "SELECT COUNT(*) FROM follow WHERE fromUserId = :pageUserId", nativeQuery = true)
    int mFollowCount(@Param("pageUserId") int pageUserId);
